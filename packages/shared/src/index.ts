...
export const IPC_VERSION = 3 as const;
...
export const sessionErrorCodeSchema = z.enum(["BROWSER_START_FAILED", "PROXY_CONNECTION_FAILED", "PROXY_AUTH_FAILED", "SECRET_STORAGE_UNAVAILABLE", "RUNNER_CRASHED", "INVALID_COMMAND", "RECORDING_FAILED", "UNKNOWN"]);
...
export const runSchema = z.object({ id: idSchema, name: z.string().min(1).max(120), diagnosticLevel: diagnosticLevelSchema, status: runStatusSchema, startedAt: timestampSchema, endedAt: timestampSchema.nullable(), environment: runEnvironmentSchema, createdAt: timestampSchema, updatedAt: timestampSchema });
