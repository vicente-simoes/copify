...
export const browserProfiles = sqliteTable("browser_profiles", {
  ...
  proxyProfileId: text("proxy_profile_id"), shippingProfileId: text("shipping_profile_id"), paymentProfileId: text("payment_profile_id"), checkoutModeOverride: text("checkout_mode_override").notNull().default("INHERIT_TARGET"), launchMode: text("launch_mode").notNull().default("PLAYWRIGHT"),
...
export const paymentProfiles = sqliteTable("payment_profiles", {
  id: text("id").primaryKey(), name: text("name").notNull().unique(), kind: text("kind").notNull(), brand: text("brand"), last4: text("last4"), expiryMonth: integer("expiry_month"), expiryYear: integer("expiry_year"), tagsJson: text("tags_json").notNull().default("[]"), payloadSecretId: text("payload_secret_id"), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull()
});
...
export const runs = sqliteTable("runs", {
  id: text("id").primaryKey(), name: text("name").notNull(), diagnosticLevel: text("diagnostic_level").notNull(), executionMode: text("execution_mode").notNull(), purchaseMode: text("purchase_mode").notNull().default("LEGACY_MANUAL"), maxCheckouts: text("max_checkouts").notNull().default("UNLIMITED"), status: text("status").notNull(),
...
export const runSessions = sqliteTable("run_sessions", {
  ...
  routeJson: text("route_json").notNull(), shippingProfileJson: text("shipping_profile_json"), paymentProfileJson: text("payment_profile_json"), checkoutMode: text("checkout_mode").notNull().default("ASSISTED"), captchaStrategy: text("captcha_strategy").notNull().default("MANUAL_HARVESTER"), captchaProviderJson: text("captcha_provider_json"), assistedEligible: integer("assisted_eligible", { mode: "boolean" }).notNull().default(false), executionState: text("execution_state").notNull().default("OBSERVING"), checkpointReason: text("checkpoint_reason"), quotaOutcome: text("quota_outcome").notNull().default("NONE"), orderIndex: integer("order_index"), status: text("status").notNull(), startedAt: integer("started_at").notNull(), endedAt: integer("ended_at"), finalErrorJson: text("final_error_json")
});
...
export const runSetups = sqliteTable("run_setups", {
  ...
  profileIdsJson: text("profile_ids_json").notNull(), sessionOverridesJson: text("session_overrides_json").notNull().default("[]"), captchaOverridesJson: text("captcha_overrides_json").notNull().default("[]"), targetId: text("target_id"), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull()
});
...
export const targets = sqliteTable("targets", {
  ...
  checkoutMode: text("checkout_mode").notNull().default("ASSISTED"), maxCheckouts: text("max_checkouts").notNull().default("UNLIMITED"), captchaStrategy: text("captcha_strategy").notNull().default("INHERIT_APP"), enabled: integer("enabled", { mode: "boolean" }).notNull().default(true), latestCheckJson: text("latest_check_json"), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull()
});
