...
describe("ProfileRepository", () => {
  ...
  it("migrates to schema 19 and turns cumulative usage into immutable deltas across counter resets", async () => {
    ...
    await repo.recordUsageSnapshot({...base,id:randomUUID(),receivedBytes:100_000_000,sentBytes:0,requestCount:1,updatedAt:now+2_000});
    expect(await repo.listRunNetworkUsage(runId)).toEqual([expect.objectContaining({receivedBytes:1_200_000_000,requestCount:5,estimatedCostMicrosUsd:1_200_000})]);
    const summary=await repo.queryCosts({period:{preset:"CUSTOM",startAt:now-60_000,endAt:now+60_000,timezoneId:"Europe/Lisbon"},groupBy:"PROVIDER",provider:null},now+3_000);expect(summary.estimatedCostMicrosUsd).toBe(1_200_000);expect(summary.estimationCoverage).toBe(1);
    // The series has to account for every micro the totals claim, in one bucket
    // per hour over a two-minute window, and stay within the declared period.
    expect(summary.seriesGranularity).toBe("HOUR");
    expect(summary.series.reduce((total,point)=>total+(point.proxyCostMicrosUsd??0),0)).toBe(1_200_000);
    expect(summary.series.reduce((total,point)=>total+point.receivedBytes+point.sentBytes,0)).toBe(1_200_000_000);
    expect(summary.series[0]!.startAt).toBeLessThanOrEqual(summary.period.startAt);
    expect(summary.series[summary.series.length-1]!.endAt).toBeGreaterThanOrEqual(summary.period.endAt);
  });

  it("stores manual and provider-confirmed reconciliation separately and deduplicates imports",async()=>{const repo=repository();const now=Date.now();await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:now-10_000,intervalEndAt:now,usedBytes:1_000,requestCount:null,billedCostMicrosUsd:100,remainingCreditMicrosUsd:900});const record={provider:"dataimpulse",intervalStartAt:now-10_000,intervalEndAt:now,receivedBytes:1_100,requestCount:2,billedCostMicrosUsd:110,planLabel:null};const first=await repo.commitProviderImport("dataimpulse","digest",[record]);const second=await repo.commitProviderImport("dataimpulse","digest",[record]);expect(first.duplicate).toBe(false);expect(second.duplicate).toBe(true);const history=await repo.listReconciliation("dataimpulse");expect(history.usage.map((row)=>row.authority).sort()).toEqual(["MANUAL_CONFIRMED","PROVIDER_CONFIRMED"]);expect(history.balances[0].remainingCreditMicrosUsd).toBe(900);});

  it("uses a manual money snapshot when an overlapping provider CSV contains traffic only",async()=>{const repo=repository();const now=Date.now();const period={preset:"CUSTOM" as const,startAt:now-10_000,endAt:now,timezoneId:"Europe/Lisbon"};await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:period.startAt,intervalEndAt:period.endAt,usedBytes:745_000_000,requestCount:null,billedCostMicrosUsd:730_000,remainingCreditMicrosUsd:4_250_000});await repo.commitProviderImport("dataimpulse","traffic-only",[{provider:"dataimpulse",intervalStartAt:period.startAt,intervalEndAt:period.endAt,receivedBytes:745_000_000,requestCount:2_437,billedCostMicrosUsd:null,planLabel:null}]);const summary=await repo.queryCosts({period,groupBy:"PROVIDER",provider:"dataimpulse"},now);expect(summary.confirmedCostMicrosUsd).toBe(730_000);expect(summary.confirmedAuthority).toBe("MANUAL_CONFIRMED");expect(summary.remainingCreditMicrosUsd).toBe(4_250_000);const subset={...period,startAt:period.startAt+1};expect((await repo.queryCosts({period:subset,groupBy:"PROVIDER",provider:"dataimpulse"},now)).confirmedCostMicrosUsd).toBeNull();const superset={...period,startAt:period.startAt-1};const expanded=await repo.queryCosts({period:superset,groupBy:"PROVIDER",provider:"dataimpulse"},now);expect(expanded.confirmedCostMicrosUsd).toBe(730_000);expect(expanded.confirmedDifferenceMicrosUsd).toBeNull();});

  it("removes only a manual usage snapshot",async()=>{const repo=repository();const now=Date.now();const {usage,balance}=await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:now-1_000,intervalEndAt:now,usedBytes:1,requestCount:null,billedCostMicrosUsd:1,remainingCreditMicrosUsd:2});expect(usage).not.toBeNull();expect(await repo.removeManualCostSnapshot(usage!.id)).toBe(true);const history=await repo.listReconciliation("dataimpulse");expect(history.usage).toHaveLength(0);expect(history.balances).toEqual([expect.objectContaining({id:balance!.id})]);});

  it("uses only the newest provider balance snapshot",async()=>{const repo=repository();const now=Date.now();await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:now-1_000,intervalEndAt:now,usedBytes:null,requestCount:null,billedCostMicrosUsd:null,remainingCreditMicrosUsd:4_250_000});await new Promise((resolve)=>setTimeout(resolve,2));await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:now-1_000,intervalEndAt:now,usedBytes:null,requestCount:null,billedCostMicrosUsd:null,remainingCreditMicrosUsd:4_270_000});const summary=await repo.queryCosts({period:{preset:"CUSTOM",startAt:now-1_000,endAt:Date.now()+1_000,timezoneId:"Europe/Lisbon"},groupBy:"PROVIDER",provider:"dataimpulse"});expect(summary.remainingCreditMicrosUsd).toBe(4_270_000);});

  it("caps a manual interval ending today at its actual observation time",async()=>{const repo=repository();const now=Date.now();const {usage}=await repo.createManualCostSnapshot({provider:"dataimpulse",intervalStartAt:now-1_000,intervalEndAt:now+86_400_000,usedBytes:1,requestCount:null,billedCostMicrosUsd:730_000,remainingCreditMicrosUsd:null});expect(usage!.intervalEndAt).toBeLessThanOrEqual(usage!.recordedAt);const summary=await repo.queryCosts({period:{preset:"CUSTOM",startAt:now-2_000,endAt:Date.now()+1_000,timezoneId:"Europe/Lisbon"},groupBy:"PROVIDER",provider:"dataimpulse"});expect(summary.confirmedCostMicrosUsd).toBe(730_000);});

  it("persists simultaneous provider budgets and threshold markers",async()=>{const repo=repository();for(const cadence of ["DAILY","WEEKLY","MONTHLY"] as const)await repo.upsertCostBudget({provider:"dataimpulse",cadence,limitMicrosUsd:10,startingCreditMicrosUsd:null,timezoneId:"Europe/Lisbon",thresholds:[50,80,100],hardCap:true,enabled:true});expect(await repo.listCostBudgets()).toHaveLength(3);const budget=(await repo.listCostBudgets())[0];expect(await repo.markBudgetThreshold(budget.id,0,50)).toBe(true);expect(await repo.markBudgetThreshold(budget.id,0,50)).toBe(false);});
  it("records one provider-reported CAPTCHA charge and includes it in CAPTCHA budgets",async()=>{const repo=repository();await repo.upsertCostBudget({category:"CAPTCHA",provider:"CAPSOLVER",cadence:"DAILY",limitMicrosUsd:10_000,startingCreditMicrosUsd:null,timezoneId:"Europe/Lisbon",thresholds:[50,80,100],hardCap:false,enabled:true});const now=Date.now()+10;const event={id:randomUUID(),runId:randomUUID(),runSessionId:randomUUID(),wallTimeMs:now,elapsedNs:"1",type:"CAPTCHA_TOKEN_ACQUIRED",stateBefore:"CAPTCHA",stateAfter:"CAPTCHA",payload:{kind:"RECAPTCHA_V3",strategy:"API_SOLVER",providerId:"CAPSOLVER",providerLabel:"CapSolver",attempt:1,costMicrosUsd:2_500,costAuthority:"PROVIDER_REPORTED",outcome:"ACQUIRED"}};expect(await repo.recordCaptchaCostEvent(event)).toBe(true);expect(await repo.recordCaptchaCostEvent(event)).toBe(false);const period={preset:"CUSTOM" as const,startAt:now-1,endAt:now+1,timezoneId:"Europe/Lisbon"};const summary=await repo.queryCosts({period,scope:"CAPTCHA",groupBy:"CAPTCHA_KIND",provider:null},now+1);expect(summary).toMatchObject({captchaCostMicrosUsd:2_500,captchaSolveCount:1,unknownCaptchaCostCount:0,totalKnownCostMicrosUsd:2_500});expect(summary.rows[0]).toMatchObject({label:"RECAPTCHA_V3",category:"CAPTCHA",captchaSolveCount:1,confirmedCostMicrosUsd:2_500,authority:"PROVIDER_REPORTED"});expect((await repo.getBudgetStatuses(now+1))[0]).toMatchObject({spentMicrosUsd:2_500,authority:"PROVIDER_REPORTED",capped:false});expect(summary.series.reduce((total,point)=>total+(point.captchaCostMicrosUsd??0),0)).toBe(2_500);expect(summary.series.reduce((total,point)=>total+point.captchaSolveCount,0)).toBe(1);});
  it("labels browser-profile and run cost breakdowns with their recorded names",async()=>{const repo=repository();const profile=await repo.create({name:"Sticky checkout"});const proxy=await repo.createProxy({name:"PT route",provider:"dataimpulse",host:"proxy.example",port:8000,costPerGbMicrosUsd:1_000_000});const now=Date.now();const runId=randomUUID();await repo.recordUsageSnapshot({id:randomUUID(),runId,usageKey:"browser:session",source:"BROWSER",runSessionId:randomUUID(),browserProfileId:profile.id,storeId:"supreme-eu",proxyProfileId:proxy.id,proxyProvider:"dataimpulse",proxyName:proxy.name,discoverySource:null,receivedBytes:1_000,sentBytes:0,requestCount:1,completeness:"PARTIAL",costPerGbMicrosUsd:1_000_000,estimatedCostMicrosUsd:null,updatedAt:now});const period={preset:"CUSTOM" as const,startAt:now-60_000,endAt:now+1_000,timezoneId:"Europe/Lisbon"};expect((await repo.queryCosts({period,groupBy:"BROWSER_PROFILE",provider:null},now)).rows[0]?.label).toBe("Sticky checkout");expect((await repo.queryCosts({period,groupBy:"RUN",provider:null},now)).rows[0]?.label).toBe(runId);});
