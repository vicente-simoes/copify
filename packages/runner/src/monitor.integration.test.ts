...
describe.skipIf(process.env.COPIFY_SUPREME_LIVE !== "1")("Supreme public HTML monitor smoke", () => {
  it("retrieves and parses the supported collection page", async () => {
    ...
    const products = parseSupremeHtmlProducts(response.body, { targetId: "00000000-0000-4000-8000-000000000001", name: "Live smoke", storeId: "supreme-eu", productKeywords: ["a"], negativeKeywords: [], directProductUrl: null, preferredColors: [], sizePriority: [], currency: "EUR", maxRetailMinor: 1_000_000, quantity: 1, checkoutMode: "ASSISTED", maxCheckouts: "UNLIMITED", captchaStrategy: "INHERIT_APP", enabled: true, capturedAt: 1 });
