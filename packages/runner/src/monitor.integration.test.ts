...
import { CrawleeJsonTransport } from "./http-monitor";
...
describe.skipIf(!enabled)("authorized/local HTTP monitor smoke", () => {
  ...
  it("uses JSON only and reuses cached data after a conditional 304", async () => { const transport = new CrawleeJsonTransport(); const first = await transport.get(endpoint, { kind: "DIRECT", id: "direct" }); const second = await transport.get(endpoint, { kind: "DIRECT", id: "direct" }); expect(first.body).toEqual({ products: [] }); expect(second.status).toBe(304); expect(second.bytes).toBe(0); expect(requests).toBe(2); }, 20_000);
  it("routes a scheduled request through the configured proxy", async () => { const transport = new CrawleeJsonTransport(); const response = await transport.get(endpoint, { kind: "PROXY", id: "00000000-0000-4000-8000-000000000001", protocol: "http", host: "127.0.0.1", port: proxyPort }); expect(response.status).toBe(200); expect(proxyConnections).toBeGreaterThan(0); }, 15_000);
  it("preserves HTML protection status and Retry-After without parsing the challenge", async () => { const response = await new CrawleeJsonTransport().get(new URL("/challenge", endpoint).toString(), { kind: "DIRECT", id: "direct" }); expect(response).toMatchObject({ status: 429, retryAfterMs: 900_000, body: {} }); }, 15_000);
});
